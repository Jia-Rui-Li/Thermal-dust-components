...
print("Dust_Planck_data_model_No_color_correction.py, Starting. ")
...
def CMB_map(freq_str, unit, Nside):
    ...
    if unit == "muK_CMB":
        ...
        # From K_CMB to MJy_sr
        CMB_map = 1e-6*1e26*b_prime_CMB(freq)*CMB_map
...
# Reading of HFI detector at each channel, in unit of muK_CMB or MJy_sr, and the output with Nside
    ...
    freq = float(freq_str)
    if unit == "muK_CMB":
        # From W m^-2 Hz^-1 sr^-1 to muK_CMB
        map_ff = 1e6*intensity_ff(freq)/b_prime_CMB(freq)
    elif unit == "MJy_sr":
        # From W m^-2 Hz^-1 sr^-1 to MJy_sr
        map_ff = 1e-6*1e26*intensity_ff(freq)
...
# Reading of HFI detector at each channel, in unit of muK_CMB or MJy_sr, and the output with Nside
    ...
    # Transform synchrotron_I from 408 MHz to freq GHz, in SI unit
    synchrotron_I_freq = synchrotron_I*synchro_intensity/synchro_intensity_ref
    if unit == "muK_CMB":
        # From SI unit to muK_CMB
        map_synchro = 1e6*synchrotron_I_freq/b_prime_CMB(freq)
    elif unit == "MJy_sr":
        # From SI unit to MJy/sr
        map_synchro = 1e-6*1e26*synchrotron_I_freq
...
def reading_data_dust(freq_str, unit, Nside):
    ...
    healpy.write_map(root_dir+"/results/No_color_correction_dust_data_"+unit+"_"+freq_str+"GHz.fits", 
...
# If year = "2015", single component model from Planck 2015 release
    ...
    # dust_intensity in SI unit
    dust_intensity = dust_tau*(freq/freq_d)**dust_index*Black_body_spectrum
    if unit == "muK_CMB":
        # From W m^-2 Hz^-1 sr^-1 to muK_CMB
        dust_map = 1e6*dust_intensity/b_prime_CMB(freq)
    elif unit == "MJy_sr":
        # From W m^-2 Hz^-1 sr^-1 to MJy_sr
        dust_map = 1e26*1e-6*dust_intensity
    ...
    healpy.write_map(root_dir+"/results/No_color_correction_dust_model_"+year+"_"+unit+"_"+freq_str+"GHz.fits", 
                     dust_map, nest = False, coord = "G", column_units = unit, overwrite="True", dtype=numpy.float64)
    return dust_map
...
print("Dust map from data (without color correction): ")
...
print("Dust map from Planck 2013 model: (without color correction): ")
start_time = time.time()
arguments = [("100", "2013", "muK_CMB", Nside), ("143", "2013", "muK_CMB", Nside), ("217", "2013", "muK_CMB", Nside), ("353", "2013", "muK_CMB", Nside), ("545", "2013", "MJy_sr", Nside), ("857", "2013", "MJy_sr", Nside)]
multiprocessing.Pool(processes=6).starmap(reading_model_dust, arguments)
end_time = time.time()
print("TIME: ", format((end_time-start_time)/60, '.2f'), "min")

print("Dust map from Planck 2015 model: (without color correction): ")
start_time = time.time()
arguments = [("100", "2015", "muK_CMB", Nside), ("143", "2015", "muK_CMB", Nside), ("217", "2015", "muK_CMB", Nside), ("353", "2015", "muK_CMB", Nside), ("545", "2015", "MJy_sr", Nside), ("857", "2015", "MJy_sr", Nside)]
multiprocessing.Pool(processes=6).starmap(reading_model_dust, arguments)
end_time = time.time()
print("TIME: ", format((end_time-start_time)/60, '.2f'), "min")

print("Dust map from Planck 2019 model: (without color correction): ")
start_time = time.time()
arguments = [("100", "2019", "muK_CMB", Nside), ("143", "2019", "muK_CMB", Nside), ("217", "2019", "muK_CMB", Nside), ("353", "2019", "muK_CMB", Nside), ("545", "2019", "MJy_sr", Nside), ("857", "2019", "MJy_sr", Nside)]
multiprocessing.Pool(processes=6).starmap(reading_model_dust, arguments)
end_time = time.time()
print("TIME: ", format((end_time-start_time)/60, '.2f'), "min")
